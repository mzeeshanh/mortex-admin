...
import { GET_ERRORS, BEFORE_EMAIL, GET_EMAILS, UPSERT_COLLECTION, GET_EMAIL, ADD_EMAIL, DELETE_EMAIL } from '../../redux/types';
...
export const updateEmail = (body) => dispatch => {
    ...
    fetch(url, {
        ...
        if (data.success) {
            toast.success(data.message, { toastId : 'email-update-success'})
            dispatch({
...
export const addEmail = (body) => dispatch => {
    dispatch(emptyError());
    const url = `${ENV.url}email/create`;
...
export const delEmail = (emailId) => dispatch => {
    dispatch(emptyError());
    const url = `${ENV.url}email/delete/${emailId}`;
